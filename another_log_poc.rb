...
File.readlines(file_path).each do |record_detail|
			...
			instance_id.push(record_detail.match(/\[]|\[i-\w*]/)[0])
			description.push(record_detail.chomp.match(/:\s\w[\w\s]*[\W\w]*/)[0])
			log_records.push([record_detail.match(/\[[0-9]\]/)[0].match(/[0-9]/)[0], record_detail.match(/[0-9]{1,4}-[0-9]{1,4}-[0-9]{1,4}T+[0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2}.[0-9]{1,6}/)[0], record_detail.match(/#[0-9]{1,4}/)[0], record_detail.match(/\[]|\[i-\w*]/)[0], record_detail.chomp.match(/:\s\w[\w\s]*/)[0].match(/\w[\w\s]*/)[0]])
		end
